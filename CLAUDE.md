...
- `co.architect.composer.expect`: `up` (default) or `manual`; `status` renders DOWN dim rather than red for `manual` services and, by default, for services behind a profile
